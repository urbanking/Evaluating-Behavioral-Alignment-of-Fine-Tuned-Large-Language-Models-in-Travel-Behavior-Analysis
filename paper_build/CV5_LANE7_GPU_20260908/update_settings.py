...
def main():
    ...
    for key,label in [('ffnn','FFNN'),('dnn','DNN')]:
        ls=compressed(values(key,'hidden'),layer)
        nn.append(label+' & Layers '+ls+r'; ReLU; dropout 0.3; AdamW, learning rate $10^{-3}$, weight decay $10^{-5}$ & Focal loss ($\gamma=1$, $\alpha=0.75$); inner-OOF Platt scaling \\')
    rows=[
    ...
    s=s[:a]+r'{\footnotesize\raggedright Note: Slash-separated settings follow folds 1--5. Neural models use batches of 512, a cosine learning-rate schedule over at most 300 epochs, and PR-AUC early stopping with patience 30 on a respondent-grouped 15\% inner validation split. All outer-fold models use seed 42. The pooled logit has no respondent random effects and is separate from H-MNL. XGBoost uses version 3.0.2; the earlier experiment did not record its package version.\par}'+s[b:]
    target='The previously fixed class-treatment rule is then applied consistently.'
    replacement='For the four non-neural candidates, the best inner-score configuration is refitted with the previously fixed class-treatment rule.'
